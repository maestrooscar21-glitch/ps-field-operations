...
st.set_page_config(
    page_title="Operações de Campo PS | Gestão",
    page_icon="🚛",
    layout="wide",
)
...
def salvar_revisao_md(
    ...
    # Invalida somente os caches afetados pela revisão MD.
    # Evita apagar todo o cache do aplicativo a cada tratativa.
    carregar_revisoes_md.clear()
...
# =========================================================
# FRAGMENTO DE TRATATIVA MD — PERFORMANCE
# =========================================================
# Widgets dentro deste fragmento não reexecutam o Dashboard de
# Improdutividade inteiro quando o usuário apenas troca a OS selecionada.
_fragment = getattr(st, "fragment", lambda func: func)

@_fragment
def render_tratativa_md(analise: pd.DataFrame, base: pd.DataFrame) -> None:
    # =====================================================
...
st.title("🚛 Operações de Campo PS — Gestão")
st.caption("Painel de Gestão Operacional de Campo")
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Versão 2.8.1 — Performance da Tratativa MD")
...
if pagina == "📥 Importações":
    ...
    st.caption(
        ...
        "são descartados do conteúdo bruto persistido. O app de Gestão não captura nem atualiza WhatsApp de oficina. "
...
# =========================================================
    ...
    # =====================================================
    # OS EXPURGADAS DO MD — VISIBILIDADE / AUDITORIA
    # =====================================================
    ...
    mascara_expurgadas_md = mascara_improdutiva_expurgada_md(filtrada)
    expurgadas_md = filtrada[mascara_expurgadas_md].copy()

    total_expurgadas_md = len(expurgadas_md)
    expurgadas_agendadas_md = int(
        (expurgadas_md.get("Classificação", pd.Series(dtype=str))
         == "Improdutiva agendada").sum()
    )
    expurgadas_extras_md = int(
        (expurgadas_md.get("Classificação", pd.Series(dtype=str))
         == "Improdutiva extra").sum()
    )

    ex1, ex2, ex3 = st.columns(3)
    ex1.metric("OS expurgadas da MD", total_expurgadas_md)
    ex2.metric("Agendadas expurgadas", expurgadas_agendadas_md)
    ex3.metric("Extras expurgadas", expurgadas_extras_md)

    if expurgadas_md.empty:
        st.info(
            "Nenhuma OS foi expurgada da MD para os filtros selecionados."
        )
    else:
        expurgadas_md["Motivo usado no expurgo MD"] = expurgadas_md.get(
            "Razao_improdutiva_md",
            expurgadas_md.get(
                "Razao_improdutiva",
                pd.Series("", index=expurgadas_md.index, dtype=str),
            ),
        ).apply(texto_limpo)

        expurgadas_md["Origem do motivo MD"] = expurgadas_md.get(
            "Revisao_MD",
            pd.Series(False, index=expurgadas_md.index),
        ).apply(
            lambda revisada: "Revisão gerencial MD" if bool(revisada) else "OFS original"
        )
        ...
        colunas_expurgadas = [
            ...
            "Tecnico_recurso",
            "Razao_improdutiva",
            "Motivo usado no expurgo MD",
            "Origem do motivo MD",
            "Justificativa_revisao_MD",
            "Revisor_MD",
        ]
        colunas_expurgadas = [
            coluna for coluna in colunas_expurgadas
            if coluna in expurgadas_md.columns
        ]

        tabela_expurgadas_md = expurgadas_md[colunas_expurgadas].copy()
        tabela_expurgadas_md = tabela_expurgadas_md.rename(
            columns={
                "OS_resultado": "OS",
                "Tecnico_recurso": "Técnico / Recurso",
        ...
        tabela_expurgadas_md = tabela_expurgadas_md.sort_values(
            [
                coluna
                for coluna in ["Data Operacional", "Oficina", "OS"]
                if coluna in tabela_expurgadas_md.columns
            ],
            ascending=True,
        ).reset_index(drop=True)

        st.dataframe(
            tabela_expurgadas_md,
            use_container_width=True,
            hide_index=True,
            height=420,
        )
        ...
        st.download_button(
            "⬇️ Baixar OS expurgadas do MD em Excel",
            data=dataframe_para_excel(
                tabela_expurgadas_md,
                "Expurgadas MD",
            ),
            file_name="os_expurgadas_md.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument."
                "spreadsheetml.sheet"
            ),
    ...
    # A tratativa roda como fragmento independente. Ao trocar apenas a OS,
    # Streamlit reexecuta somente este bloco, sem reconstruir rankings/gráficos.
    render_tratativa_md(analise, base)
